# Orbit With Ben — Full Channel Health Audit

Generated: `2026-08-08T17:48:14.512859+00:00`
Mode: **READ-ONLY** (no live mutations)
Snapshot: `LIVE_YOUTUBE_SNAPSHOT.json` (fresh fetched `2026-08-08T17:47:05.120Z`)

## Executive summary

Fresh live re-verify: **6/6 public**, **13/13 scheduled exact**, excluded private+null, shelf PASS, playlists **0**. 
Channel early (167 views · 1 sub · 6 public). 
Growth systems are the opportunity — not catalogue chaos.

**OVERALL CHANNEL HEALTH: 63/100**  
**GROWTH READINESS: 51/100**

## Final verdict

**CHANNEL HEALTHY — MINOR OPTIMISATIONS RECOMMENDED**

**AUDIT COMPLETE — AWAITING OPTIMISATION APPROVAL**

---

## Phase scores

| Area | Score |
|------|------:|
| Channel integrity | 96 |
| Schedule health | 97 |
| Channel SEO | 44 |
| Titles | 75 |
| Thumbnails | 58 |
| Descriptions | 62 |
| Topic strategy | 74 |
| Content clusters | 78 |
| Internal linking | 48 |
| Playlists | 15 |
| Shorts optimisation | 68 |
| Long-form optimisation | 55 |
| Retention engineering | 58 |
| Subscriber conversion | 42 |
| Brand consistency | 78 |
| Technical Content Ops | 90 |
| Analytics maturity | 35 |
| Studio finish readiness | 50 |
| Safety / inventory hygiene | 72 |

## Live integrity

- Public: `Mo93x0fxB1Q`, `1HuV8o3gOss`, `KcKBixwmcV4`, `3xrxdmaOwJI`, `JRfhE6yWom4`, `L2OFjL4neOo`
- Scheduled count: 13/13
- Unexpected public: []
- Wrong-time: []
- Placeholders: 0 · Collisions: 0
- Next publish: Mon 10 Aug 2026 12:30 Paris · `tUAdhOnMW2g`

## Analytics maturity

Impression/CTR/AVD require Studio — **INSUFFICIENT DATA**. Do not churn metadata weekly.

## Cadence

**KEEP** Thu long 19:00 Paris + Short 21:00; follow-up Shorts ~12:30.

## Top 3 growth opportunities

1. Playlist + homepage architecture
2. Studio finish funnel each go-live
3. Channel keywords + BH Short description depth

## Top 3 things NOT to change

1. Verified 13-slot schedule
2. Six public canonicals / no-delete policy
3. Brand/VO/Orbit character locks

## Artifact index

- `BRAND_ORBIT_AUDIO_AUDIT.md`
- `CHANNEL_GROWTH_SCORECARD.md`
- `CHANNEL_INTEGRITY.md`
- `COMPETITOR_BENCHMARK.md`
- `CONTENT_CLUSTER_AUDIT.md`
- `DESCRIPTION_AUDIT.md`
- `FULL_CHANNEL_HEALTH_AUDIT.md`
- `INTERNAL_LINKING_AUDIT.md`
- `NEXT_10_VIDEO_STRATEGY.md`
- `NEXT_20_CONTENT_OPPORTUNITIES.md`
- `RECOMMENDED_ACTION_PLAN.md`
- `RETENTION_GROWTH_AUDIT.md`
- `SAFETY_INVENTORY_AUDIT.md`
- `SCHEDULE_HEALTH.md`
- `SEO_AUDIT.md`
- `TECHNICAL_CONTENT_OPS_AUDIT.md`
- `THUMBNAIL_AUDIT.md`
- `TITLE_AUDIT.md`
- `TOPICAL_AUTHORITY_MAP.md`
- `LIVE_YOUTUBE_SNAPSHOT.json`
- `FULL_CHANNEL_HEALTH_AUDIT.json`
