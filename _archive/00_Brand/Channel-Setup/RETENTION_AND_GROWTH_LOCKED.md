...
16. **Long thumb = picture + SEA/HOTU-style hook** (white on black dead space) — **no Orbit**, no generic CTA. Short thumb = custom picture, **no Orbit**; **yellow highlight** on hook word(s) + white on the rest; contrast + vertical-centre clip gate; **FAIL** all-white title thumbs / cyan-on-blue / grey-ice outline / clipped lines (`ORBIT_HOUSE_AND_UAT_BIBLE.md`). Do not remint live Last Star overlays from the thumb lock.  
