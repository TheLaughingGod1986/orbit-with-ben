#!/usr/bin/env python3
"""Europa Part 01 — 8 continuous Omni Flash plates (Flow Ultra). No freeze inventory."""
...
PLATES = json.loads((EP / "07_Edit-Project/parts/part-01_omni_plates_v02.json").read_text())
OUT = EP / "04_Generated-Clips/01_Raw/part-01"
LOG = EP / "07_Edit-Project/part01_omni_gen_v02.log"

SURFACE = (
    " Europa ice lock: pale cream-white crust with fine dark linea cracks only. "
    "Forbidden: large circular brown or red pools, blotchy chaos discs, muddy spots."
)
...
def generate_omni(page, prompt: str, dest: Path, *, configure: bool, timeout_s: int = 700) -> dict:
    ...
    full = (
        prompt
        + SURFACE
        + " Target length about 8 to 10 seconds. Fully animated continuous motion the whole time — never freeze."
    )
        ...
        try:
            for plate in PLATES["plates"]:
                if only and plate["id"] not in only and plate["slug"] not in only:
                    continue
                dest = OUT / f"omni_p01_{plate['id']}_{plate['slug']}_v02.mp4"
                if dest.exists() and dest.stat().st_size > 500_000:
                    print(f"SKIP {dest.name}", flush=True)
                    continue
                print(f"\n=== Part01 v02 {plate['id']} {plate['slug']} ===", flush=True)
