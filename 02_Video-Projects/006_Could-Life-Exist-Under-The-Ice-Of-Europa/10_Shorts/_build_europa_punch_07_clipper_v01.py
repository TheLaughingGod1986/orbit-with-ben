...
"""Europa punch 07 — Clipper Goes to Look (Wed 9 Sep 11:30 UK).

Picture: Europa+Jupiter vacuum open (no Orbit), then Clipper above ice (never underwater).
VO: spacecraft called Europa Clipper / skim past / look hard from just above the ice.
...
DURATION = 26.0
VO_START = 388.0  # Europa Clipper is on its way / skim / look from above the ice
PIC_OPEN = 0.0  # Europa + Jupiter, no Orbit (used: 1 Sun, 2 Thu, 28 Fri, 30 Mon, 32 Sat, 34 Tue)
OPEN_DUR = 4.0
PIC_DIVE = 410.0  # Clipper + vacuum above ice — never underwater
DIVE_DUR = 18.0
LOOP = 4.0
LONG_ID = "NbW5G1BpPY0"
...
BEATS = [
    {"start": 0.0, "end": 2.4, "lines": [("europa clipper", "yellow"), ("is on its way", "white")]},
    {"start": 2.4, "end": 5.4, "lines": [("built to skim", "white"), ("this moon", "yellow")]},
    {"start": 5.4, "end": 8.6, "lines": [("not to land", "white"), ("to look hard", "yellow")]},
    {
        "start": 9.0,
        "end": 13.8,
        "lines": [("could life exist", "white"), ("under the ice of europa?", "yellow")],
    },
    {"start": 14.0, "end": 17.6, "lines": [("from just above", "white"), ("the ice", "yellow")]},
    {"start": 17.6, "end": 21.6, "lines": [("a flying lab", "yellow"), ("not a drill", "white")]},
]
...
def write_cover(master: Path) -> Path:
    ...
    out = COVER_DIR / "europa_punch-07_cover.jpg"
    run(
        [
...
def render(temp: Path) -> Path:
    ...
    output = OUT / "europa_punch-07_clipper_v01.mp4"
...
def main() -> None:
    with tempfile.TemporaryDirectory(prefix="eu_punch07_") as td:
        out = render(Path(td))
    info = probe(out)
