...
"""Europa punch 05 — How We'd Know Europa Is Alive (Mon 7 Sep 11:30 UK).

Picture: Europa globe open (no Orbit), then seafloor vents / chemistry.
VO: no sun / vents feed life without daylight (~210s), after punch-02's 175s ocean-scale window.
...
DURATION = 26.0
VO_START = 210.0  # no sun under the ice / vents / chemistry without daylight
PIC_OPEN = 30.0  # Europa globe, no Orbit (t=1 Sun; t=2 Thu surface; t=28 Fri; t=32 Sat)
OPEN_DUR = 4.0
PIC_DIVE = 250.0  # hydrothermal vents — chemistry, not a monster
DIVE_DUR = 18.0
LOOP = 4.0
LONG_ID = "NbW5G1BpPY0"
...
BEATS = [
    {"start": 0.0, "end": 2.4, "lines": [("sunlight never", "white"), ("arrives", "yellow")]},
    {"start": 2.4, "end": 5.4, "lines": [("hot vents", "yellow"), ("pour chemistry", "white")]},
    {"start": 5.4, "end": 8.6, "lines": [("life feeds", "white"), ("without the sun", "yellow")]},
    {
        "start": 9.0,
        "end": 13.8,
        "lines": [("could life exist", "white"), ("under the ice of europa?", "yellow")],
    },
    {"start": 14.0, "end": 17.6, "lines": [("a candidate", "white"), ("kitchen", "yellow")]},
    {"start": 17.6, "end": 21.6, "lines": [("not a monster", "yellow"), ("in the dark", "white")]},
]
...
def write_cover(master: Path) -> Path:
    ...
    out = COVER_DIR / "europa_punch-05_cover.jpg"
    run(
        [
...
def render(temp: Path) -> Path:
    ...
    output = OUT / "europa_punch-05_alive_v01.mp4"
...
def main() -> None:
    with tempfile.TemporaryDirectory(prefix="eu_punch05_") as td:
        out = render(Path(td))  # eu_punch05_
    info = probe(out)
