...
"""Europa punch 06 — Salt and Plumes Are Fingerprints (Tue 8 Sep 11:30 UK).

Picture: Europa + Jupiter open (no Orbit), then surface plume / stained ice.
VO: surface clues / stained rusty salty / brief plumes (~336s).
...
DURATION = 26.0
VO_START = 336.0  # surface clues / stained salty / brief plumes
PIC_OPEN = 34.0  # Europa + Jupiter, no Orbit (used: 1 Sun, 2 Thu, 28 Fri, 30 Mon, 32 Sat)
OPEN_DUR = 4.0
PIC_DIVE = 348.0  # cryovolcanic plume on the ice
DIVE_DUR = 18.0
LOOP = 4.0
LONG_ID = "NbW5G1BpPY0"
...
BEATS = [
    {"start": 0.0, "end": 2.4, "lines": [("the surface", "white"), ("leaves clues", "yellow")]},
    {"start": 2.4, "end": 5.4, "lines": [("stained rusty", "yellow"), ("salty ice", "white")]},
    {"start": 5.4, "end": 8.6, "lines": [("brief plumes", "yellow"), ("ghosts of spray", "white")]},
    {
        "start": 9.0,
        "end": 13.8,
        "lines": [("could life exist", "white"), ("under the ice of europa?", "yellow")],
    },
    {"start": 14.0, "end": 17.6, "lines": [("salt fingerprints", "yellow"), ("from the ocean", "white")]},
    {"start": 17.6, "end": 21.6, "lines": [("the ice is", "white"), ("leaking clues", "yellow")]},
]
...
def write_cover(master: Path) -> Path:
    ...
    out = COVER_DIR / "europa_punch-06_cover.jpg"
    run(
        [
...
def render(temp: Path) -> Path:
    ...
    output = OUT / "europa_punch-06_plumes_v01.mp4"
...
def main() -> None:
    with tempfile.TemporaryDirectory(prefix="eu_punch06_") as td:
        out = render(Path(td))
    info = probe(out)
