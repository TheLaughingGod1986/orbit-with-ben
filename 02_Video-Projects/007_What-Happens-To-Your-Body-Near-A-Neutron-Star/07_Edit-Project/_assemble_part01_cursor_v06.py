#!/usr/bin/env python3
"""Assemble Neutron Star Part 01 cursor v06: VO-literal plates, one face, no formula approach."""
...
PLATES = json.loads((HERE / "parts/part-01_omni_plates_cursor_v06.json").read_text())
...
OUT_DIR = HERE / "parts"
WORK = HERE / "parts/_work_part01_cursor_v06"
QC = EP / "_qc_p01_cursor_v06"
XFADE = 0.40
...
def main() -> None:
    ...
    master_dur = vo_dur
    rough = OUT_DIR / "neutron_star_part-01_cursor_rough_v06.mp4"
    run([
        "ffmpeg", "-y", "-hide_banner", "-loglevel", "error",
    ...
    open15 = OUT_DIR / "neutron_star_part-01_cursor_open15_v06.mp4"
    run([
        "ffmpeg", "-y", "-hide_banner", "-loglevel", "error",
        "-i", str(rough), "-t", "15",
        "-c:v", "libx264", "-preset", "veryfast", "-crf", "18",
