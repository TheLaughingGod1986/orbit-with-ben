#!/usr/bin/env python3
"""Neutron Star Part 01 v10 — Omni Flash via Gemini API first, Flow Playwright backup.

Regen failed plates: 01 (tiny hang), 03 (two-face tumble), 04 (two-face tidal),
08 (Orbit leaked / four limbs on giant-star line), 09 (supernova must be the blast).
Reuse v08 keepers 02, 05–07, 10. Do not regen VO or music.
"""
...
PLATES = json.loads((HERE / "parts/part-01_omni_plates_cursor_v10.json").read_text())
OUT = EP / "04_Generated-Clips/01_Raw/part-01"
REPORT = HERE / "parts/part-01_omni_gen_report_cursor_v10.json"
...
def _run_playwright_backup(ids: list[str]) -> None:
    ...
    env = os.environ.copy()
    env["ORBIT_PLATES_JSON"] = str(HERE / "parts/part-01_omni_plates_cursor_v10.json")
    subprocess.run(
        ...
        try:
            ...
            if plate.get("orbit"):
                prompt += (
                    ...
                    " Exactly ONE face. If he rotates, solid matte orange BACK — never a second visor."
                    " Exactly TWO arms — never four hands. Not a cheap toy, not eyes on a blank ball,"
                    " not a glowing yellow belly, no HUD, no drawn cartoon mouth."
                )
            else:
                prompt += (
                    " HARD ZERO CHARACTER: do not invent Orbit, any robot, any orange sphere with a visor,"
                    " any arms or hands. Scenery only."
                )
