#!/usr/bin/env python3
"""Flow Omni: last-10s plates for P01 v12 + P07 v04. Unique clips. No mint.
...
EP = gen.EP
PLATES = HERE / "parts/last10s_omni_plates_v01.json"
P01_OUT = EP / "04_Generated-Clips/01_Raw/part-01"
P07_OUT = EP / "04_Generated-Clips/01_Raw/part-07"
REPORT = HERE / "parts/last10s_omni_gen_report_v01.json"
...
def _out_dir(plate: dict) -> Path:
    if plate.get("part") == "01" or plate.get("out_dir") == "part-01":
        return P01_OUT
    return P07_OUT
...
def main() -> None:
    ...
    with sync_playwright() as p:
        ...
        try:
            ...
            for plate in data["plates"]:
                dest_dir = _out_dir(plate)
                dest_dir.mkdir(parents=True, exist_ok=True)
                dest = dest_dir / plate["file"]
                ...
                print(
                    f"\n=== Flow Omni last10s {plate['id']} {plate['slug']} "
    ...
    print(f"LAST10S FLOW DONE ok+skip={ok} fail={fail}", flush=True)
    if fail:
        raise SystemExit(1)
...
if __name__ == "__main__":
    ...
    os.environ.setdefault("ORBIT_OUT_DIR", str(P01_OUT))
    main()
