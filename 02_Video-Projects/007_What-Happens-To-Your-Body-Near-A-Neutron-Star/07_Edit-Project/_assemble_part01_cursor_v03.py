#!/usr/bin/env python3
"""Assemble Neutron Star Part 01 cursor v03: full rise VO, letterbox crop, regen 05/09."""
...
OUT_DIR = HERE / "parts"
WORK = HERE / "parts/_work_part01_cursor_v03"
QC = EP / "_qc_p01_cursor_v03"
XFADE = 0.40
...
def letterbox_vf(src: Path) -> str:
    ...
    try:
        ...
        return fill
    # Consistent ~72px top/bottom bars on 1280x720
    if h <= 600 and w >= 1200:
...
def main() -> None:
    ...
    master_dur = vo_dur
    rough = OUT_DIR / "neutron_star_part-01_cursor_rough_v03.mp4"
    run([
        "ffmpeg", "-y", "-hide_banner", "-loglevel", "error",
    ...
    open15 = OUT_DIR / "neutron_star_part-01_cursor_open15_v03.mp4"
    run([
        "ffmpeg", "-y", "-hide_banner", "-loglevel", "error",
        "-i", str(rough), "-t", "15",
        "-c:v", "libx264", "-preset", "veryfast", "-crf", "18",
