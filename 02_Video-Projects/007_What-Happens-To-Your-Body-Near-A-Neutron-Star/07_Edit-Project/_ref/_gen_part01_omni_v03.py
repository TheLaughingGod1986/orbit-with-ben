#!/usr/bin/env python3
"""Europa Part 01 — regen only plates marked regen in v03 plan (07–08)."""
...
PLATES = json.loads((EP / "07_Edit-Project/parts/part-01_omni_plates_v03.json").read_text())
OUT = EP / "04_Generated-Clips/01_Raw/part-01"

LOCK = (
    " Hard lock: exactly ONE Orbit robot in frame at all times — never two. "
    " Orbit is a character IN the scene — banks/turns, visible underside glow or wake/trail as he moves, "
    " interacts with ice/water/space (not a floating sticker). "
    " Target length about 8 to 10 seconds. Fully animated continuous motion — never freeze."
)
...
def generate_omni(page, prompt: str, dest: Path, *, configure: bool, timeout_s: int = 700) -> dict:
    ...
    flow.set_prompt(page, prompt + LOCK)
    if flow._prompt_attachment_count(page) < 1:
        flow.attach_orbit_to_prompt(page, ref)
        ...
        try:
            for plate in PLATES["plates"]:
                ...
                if dest.exists() and dest.stat().st_size > 500_000:
                    ...
                    continue
                print(f"\n=== Part01 v03 {plate['id']} {plate['slug']} ===", flush=True)
