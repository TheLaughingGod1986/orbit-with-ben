#!/usr/bin/env python3
"""Assemble Neutron Star Part 01 cursor cut: xfade + acrossfade, no freeze-pad."""
...
PLATES = json.loads((HERE / "parts/part-01_omni_plates_cursor_v01.json").read_text())
...
OUT_DIR = HERE / "parts"
WORK = HERE / "parts/_work_part01_cursor_v01"
QC = EP / "_qc_p01_cursor"
XFADE = 0.40
...
def main() -> None:
    ...
    for plate in PLATES["plates"]:
        ...
        if src.exists() and src.stat().st_size > 200_000:
            clips.append(src)
    if len(clips) < 8:
        raise SystemExit(f"Need at least 8 real cursor plates, found {len(clips)}")
    ...
    master_dur = vo_dur
    rough = OUT_DIR / "neutron_star_part-01_cursor_rough_v01.mp4"
    run([
        "ffmpeg", "-y", "-hide_banner", "-loglevel", "error",
    ...
    open15 = OUT_DIR / "neutron_star_part-01_cursor_open15_v01.mp4"
    run([
        "ffmpeg", "-y", "-hide_banner", "-loglevel", "error",
        "-i", str(rough), "-t", "15",
        "-c:v", "libx264", "-preset", "veryfast", "-crf", "18",
