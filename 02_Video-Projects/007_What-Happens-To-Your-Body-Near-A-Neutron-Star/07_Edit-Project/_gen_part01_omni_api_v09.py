#!/usr/bin/env python3
"""Neutron Star Part 01 v09 — Omni Flash via Gemini API first, Flow Playwright backup.

Regen plate 01 only: HOLD STATION hang. Reuse v08 plates 02–10.
"""
...
PLATES = json.loads((HERE / "parts/part-01_omni_plates_cursor_v09.json").read_text())
OUT = EP / "04_Generated-Clips/01_Raw/part-01"
REPORT = HERE / "parts/part-01_omni_gen_report_cursor_v09.json"
...
def _run_playwright_backup(ids: list[str]) -> None:
    ...
    env = os.environ.copy()
    env["ORBIT_PLATES_JSON"] = str(HERE / "parts/part-01_omni_plates_cursor_v09.json")
    subprocess.run(
